...
class DashBoardDao {
    ...
    public function getServiciosConvenio()
    {
        ...
        $fecha = $anio."-".$mes."-".$dia;
        try {
            $query = "SELECT COUNT(*) AS servicio_cantidad,servicio_cliente FROM tbl_servicio WHERE DATE_FORMAT(servicio_fecha, '%Y-%m-%d') = '$fecha' AND servicio_estado = 5 GROUP BY servicio_cliente";
            $conn->conectar();
            $result = mysqli_query($conn->conn,$query) or die (mysqli_error($conn->conn)); 
            while($row = mysqli_fetch_array($result)) {
